...
{
	// Open Image File
	ifstream ifile("pikachu.bmp", ios::binary);
	...
	// Write New Image
	ofstream of1("Q4.bmp", ios::binary);	
	...
	for(int i=0; i<height; i++)
	{
		...
		for(int j=0; j<width; j++)
		{
			...
			for(int k=0; k<bits_px/8; k++)
			{
				of1.write((char*)&rgb2[i_temp][j_temp][k], sizeof(rgb2[i_temp][j_temp][k]));
			}
		}
